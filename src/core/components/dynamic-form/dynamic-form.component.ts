...
export class DynamicFormComponent implements OnChanges {
  ...
  private onSubmit() {
    ...
    for (var prop in this.form.value) {
        // get constructor name. IE handles it different.
        let funcName = this.form.value[prop].constructor.name || this.form.value[prop].constructor.toString().match(/function (.{1,})\(/)[1]);

        if (typeof this.form.value[prop] === 'object' && funcName === 'RadioButtonState') {
    ...
    //if submit button is present, disable it.
        ...
        formElement.validations.map((validation:FormElementValidation)=> {
          switch (FormElementValidationType[validation.type]) {
            ...
            case FormElementValidationType.equivalent:
              validations.push(this.equivalent(formElement.key, validation.subKeys, false));
              break;
            case FormElementValidationType.notEquivalent:
              validations.push(this.equivalent(formElement.key, validation.subKeys, true));
              break;
            case FormElementValidationType.fileType:
              validations.push(this.fileType(validation.expression));
              break;
          }
        });
      }
  ...
  private fileType(type: string){
      return (group: ControlGroup) => {
          let ifValid = group.value.indexOf(type) !== -1;
          return ifValid ? null : {fileType: false};
      }
  }


  //a validation to check if multiple fields are equal or not equal
  private equivalent(orignalKey: string, otherKeys: array, shouldMatch: boolean) {
    return (group: ControlGroup) => {
      let error = null
      let i = 0;
      ...
      let ifMatch = function(scope){
        if(otherKeys){
          for(i;i<otherKeys.length;i++){
            ...
            let controls = scope.form ? scope.form.controls : 0;
            let otherKey = controls && controls.hasOwnProperty(otherKeys[i]) ? scope.form.controls[otherKeys[i]] : 0
            let otherValue = otherKey ? otherKey.value.split(" ").join("").toLowerCase() : ""
  ...
  //updates dropdowns based off of another dropdown
  ...
  private updateHiddenField(value:String,options:Array,subControl:array){
      ...
      //toggle out the value since it may have validations. To Do: I feel like there is a better way to toggle validation
  ...
  private formChangeEvent (change) {
    ...
    if (change.target.getAttribute('type') === 'file') {
      let file = change.target.files[0] || {name: ""}
      ...
      if(config.validations){
        let validation = config.validations[_.map(config.validations, 'type').indexOf('fileType')];
        let expression = validation ? validation.expression : "";
        let ctrlElem = this.form.controls[formElement];

        //is there an expression
        if(expression.length){
            //set validation. Does file extension right? Is there even a file uploaded?
            let error = file.name.indexOf(expression) !== -1 ? null : {fileType: false};
            ctrlElem.setErrors(error);
            ctrlElem.markAsDirty();
        }
      }

      //only do this is file name exist
      if(file.name !== "") reader.readAsText(file);
    }
    ...
    if (config && config.subControl) {
      ...
      for(var i = 0;i<config.subControl.length;i++){
        ...
        //if subStatus is a property, update status of subcontrolled item
