...
export class WorkflowComponent implements OnInit, OnDestroy {
  ...
  @Output()
  private isCanceled = new EventEmitter<boolean>();
  ...
  private wfExecutions:Observable<any>;
  private workflowConvetextObs:Observable<any>;
  ...
  ngOnDestroy():any {
    ...
    this.workflowConvetextObs.unsubscribe();
    this.wfExecutions.unsubscribe();
  }
  ...
  private listenForEvents():void {
    this.wfExecutions = this._store.select('WorkflowExecutionReducer');
    this.workflowConvetextObs = this._store.select('WorkflowContextReducer');
    this.wfExecutions.subscribe(
        ...
        if (wfExecs && wfExecs.length > 0) {
          wfExecs.map((wfExec:WorkflowExecution)=> {
            if (this._wfExec.id == wfExec.id) {
              ...
              if (this._executionStatus == WorkflowExecutionStatus.CANCELED || this._executionStatus == WorkflowExecutionStatus.TERMINATED)
            	  return;
              ...
              if (this._executionStatus != WorkflowExecutionStatus.CANCELED)
   ...
   * @param stepId
   */
  private gotoStep(stepId:string, name:string):void {
...
//if you click on the first step, don't splice anyting.
  ...
  private cancelWorkflow() {
    ...
    this._executionStatus = WorkflowExecutionStatus.CANCELED;
    this.workflowConvetextObs.unsubscribe();
    this.wfExecutions.unsubscribe();
