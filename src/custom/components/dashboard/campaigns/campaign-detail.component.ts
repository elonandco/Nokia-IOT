import {Component,OnInit,OnDestroy,ReflectiveInjector,ElementRef,Renderer} from "@angular/core";
...
export class CampaignDetailComponent implements OnInit, OnDestroy{
  ...
  constructor(private _router:Router, public _campaignService: CampaignService, private _store:Store<any>, data:RouteData,private _drc:DynamicRouteConfiguratorService,
    ...
    this.dsoObsubscription = this.dsoObs.subscribe((data:any)=> {
      ...
      if (Campaigns != undefined && Campaigns.operations != undefined){
        ...
        this.addCampaignOperation = _.remove(operations,function(operation){
  ...
  initDetails(){
    ...
    this.resultStatus = this._campaignService.listResultStatus;
    this.resultMessage = this._campaignService.listResultMessage;
    this.resultCode = this._campaignService.listResultCode; 

    this.initializePaginatorInfo();
	   ...
	   this._wfeLaunchService.isDone.subscribe((data:any)=> {
		   if (data){
			   ...
			   this.options.series[0].data = JSON.parse(
					   				'[{"name":"Submitted", "y":' + response.submittedCampaignCount + ',"color":"#1F77B4"},'+ 
  ...
  initializePaginatorInfo(){
  
  if(this._allCampaigns && this.status !== 'FAIL'){
