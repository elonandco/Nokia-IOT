...
columnData:Array<any> = [];
  ...
  onClose(evt:Event) {
    let workflow = this._store._value.WorkflowExecutionReducer;
    let data = workflow[workflow.length - 1].step.content.data
    let ifLastPage = data ? data.breadcrumbStatus === "disabled" : false;
    let ifSuccess = workflow[workflow.length - 1].step.displayName === 'Success!';

    if (ifLastPage && ifSuccess){
        this.logout();
    }else{
        let link = ['Dashboard'];
        this._router.navigate(link);
