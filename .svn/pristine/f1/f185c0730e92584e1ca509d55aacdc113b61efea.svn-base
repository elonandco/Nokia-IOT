...
export class QuickLinkService implements AbstractService {
  ...
  onInitCompleted(data){
    if(data && data.hasOwnProperty('status') && data.status === 'completed'){
      let quickLinksDomain = _.find(this._store.value.DSOModelReducer.domains.domain,{'id':'QuickLinks'});
      
      if (!quickLinksDomain){
    	  return;
      }
      let serviceQuickLinks:Array<any> = quickLinksDomain.services.service;
