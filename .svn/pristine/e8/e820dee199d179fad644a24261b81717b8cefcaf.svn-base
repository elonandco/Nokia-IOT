  ...
  initGraph(){
    ...
    if(all_campaigns){
      if(all_campaigns.length){
            let campaigns = all_campaigns[_.map(all_campaigns, 'type').indexOf(this.displayType)];
            if(campaigns.dates.length){
          ...
          this.status = "FAIL";
          this.message = "No Data Available for " + this.displayType;
         }
        }
      }
    }
